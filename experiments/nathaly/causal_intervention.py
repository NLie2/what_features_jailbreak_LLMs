...
for exp in exp_config: 
  ...
  for idx, row in df.iloc[0:20].iterrows():
  ...
  path = f"/data/nathalie_maria_kirch/ERA_Fellowship/datasets/causal_intervention_test_{''.join(map(str, exp["layers_to_intervene"]))}_strenght={exp["strength"]}.pkl"
  pd.to_pickle(path)
