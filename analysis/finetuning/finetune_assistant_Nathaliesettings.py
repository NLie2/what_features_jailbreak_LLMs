...
from trl import SFTConfig, SFTTrainer, DataCollatorForCompletionOnlyLM, DataCollatorForLanguageModeling
...
def main():
    ...
    specifier = 'assistantonly'
    model, tokenizer = get_model_and_tokenizer(model_name)
    model = setup_lora_model(model)

    df1 = pd.read_csv(f"/home/weisser/what_features_jailbreak_LLMs/datasets/finetune/{model_name.replace('/','_')}/jailbreak_with_refusals_Nathaliesettings.csv")
    ...
    if CHATTEMPLATE:
        ...
        print("ACTUALLY TRAINING ON USER AND ASSISTANT MESSAGES FOR NATHALIE SETTINGS, because we dont have a chat template and dont separate user and assistant messages")
        data_collator = DataCollatorForLanguageModeling(tokenizer, mlm=False)
        assert False, "Have not implemented joining messages to a single string; So far we just ran the Nathaliesettings ASR calculation on the model trained with the chat template"
